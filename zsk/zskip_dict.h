...
public:
  ...
  void zslRange(int64_t min, int64_t max, bool reverse,
...
template <typename T> class zslDict {
public:
  ...
  unsigned int ZslGetRankNo(std::string key, bool reverse = true) {
    ...
    unsigned int rankNo =
        m_Zsl->zslGetNodeByRankNo(it->second->getScore(), key);
  ...
  void zslRange(int min, int max,
                std::function<bool(std::string key, T ele)> callback,
                bool reverse = true) {
