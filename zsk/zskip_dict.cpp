...
namespace zsk {
    zskipNode::zskipNode(int level, uint64_t score, std::string value) : m_Score(score), m_Value(value), m_Backward(nullptr) {
    ...
    zskipNode* zskipList::zslInsert(uint64_t score, std::string value) {
        ...
        for (int i = m_Level - 1; i >= 0; i--) {
            ...
            while (head->m_Level[i]->m_Forward && head->m_Level[i]->m_Forward->m_Score < score ||
                (head->m_Level[i]->m_Forward && head->m_Level[i]->m_Forward->m_Score == score && head->m_Level[i]->m_Forward->m_Value < value)) {
    ...
    int64_t zskipList::zslGetNodeByRankNo(uint64_t score, std::string value)
    {
        ...
        for (int i = m_Level - 1; i >= 0; i--) {
            while (head->m_Level[i]->m_Forward &&
                (head->m_Level[i]->m_Forward->m_Score < score ||(head->m_Level[i]->m_Forward->m_Score == score && head->m_Level[i]->m_Forward->m_Value <= value))) {
