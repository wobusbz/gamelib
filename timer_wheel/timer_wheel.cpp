...
void TimerBucket::remove(Timer* timer) {
    m_timers.erase(std::find_if(m_timers.begin(), m_timers.end(), [&](Timer* tm) -> bool { return tm == timer; }));
}
...
int TimerWheel::timerFunc(int id, int64_t duration, Timer::timerFunc cb) {
    ...
    Timer* newTimer = new Timer(id, (rounds - m_ticks) / m_wheels.size(), deadlien, cb);
...
void TimerWheel::purge() {
    for (auto buckets : m_wheels) {
        for (auto it = buckets->m_timers.begin(); it != buckets->m_timers.end(); ++it) {
            if (!*it) {
                continue;
            }
            delete *it;
