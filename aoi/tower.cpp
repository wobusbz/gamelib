...
int aoi::TowerObj::X() const { return m_x; }

int aoi::TowerObj::Y() const { return m_y; }

int aoi::TowerObj::Dist() const { return m_dist; }

aoi::TowerAOIMannger::TowerAOIMannger(int minX, int maxX, int minY, int maxY,
                                      int range)
    : m_minX(minX), m_maxX(maxX), m_minY(minY), m_maxY(maxY), m_range(range),
      m_xTowerNum(0), m_yTowerNum(0) {
  init();
}
...
void aoi::TowerAOIMannger::visitWatchedTowers(
    int x, int y, int range, std::function<void(TowerAOI *)> callback) {
  if (!callback) {
    return;
  }
