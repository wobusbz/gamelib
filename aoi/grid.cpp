...
aoi::GridAOIMannger::GridAOIMannger(int minX, int minY, int maxX, int maxY,
                                    int xSize, int ySize)
    : m_minX(minX), m_minY(minY), m_maxX(maxX), m_maxY(maxY), m_xSize(xSize),
      m_ySize(ySize) {
  initGrid();
}
...
bool aoi::GridAOIMannger::visitWatchedGridObjs(
    TowerObj *obj, std::function<void(Grid *)> cbFunc) {
  auto itGrids = m_grids.find(GridID(obj->X(), obj->Y()));
...
bool aoi::GridAOIMannger::visitWatchedGridObjs(
    TowerObj *obj, std::unordered_map<uint64_t, TowerObj *> &objs) {
...
bool aoi::GridAOIMannger::visitWatchedGridObjs(TowerObj *obj,
                                               std::vector<TowerObj *> &objs) {
  if (!visitWatchedGridObjs(obj, [&](Grid *gridObj) {
        for (auto &itGridObj : gridObj->m_objs) {
          if (itGridObj.second->ID() == obj->ID()) {
